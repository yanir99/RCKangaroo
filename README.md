...
<b>-pubfile</b>		path to a file with multiple public keys and their subtracted decimal offsets. Each line must contain "<public_key> # <decimal_offset>", where the first line has offset 0 for the original public key and the following lines contain keys generated by subtracting the listed value from the original private key. The loader streams entries instead of preloading the entire file, so very large lists begin processing immediately. During solving the program walks the file in order and performs a full wild/tame search over the requested range for each entry. Wild/tame tables are rebuilt per entry (they are not shared across keys), and the solver moves to the next line only after finishing the current one or finding a match, at which point it reports the matched key and restores the original key using the offset. 
