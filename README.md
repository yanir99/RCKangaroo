...
<b>-pubfile</b>		path to a file with multiple public keys and their subtracted decimal offsets. Each line must contain "<public_key> # <decimal_offset>", where the first line has offset 0 for the original public key and the following lines contain keys generated by subtracting the listed value from the original private key. All entries are loaded up front and every distinguished point produced during solving is checked against every listed public key in the file. A match reports the paired public/private key and also restores the original key using the offset from the first line. 
