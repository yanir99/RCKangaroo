...
<b>-pubfile</b>		path to a file with multiple public keys and their subtracted decimal offsets. Each line must contain "<public_key> # <decimal_offset>", where the first line has offset 0 for the original public key and the following lines contain keys generated by subtracting the listed value from the original private key. The loader streams entries instead of preloading the entire file, so very large lists begin processing immediately. When a match is found, the software reports the matched key and restores the original key using the offset. 
